...
{
   ll n,i,j,cnt=0;
   cin>>n; vector<ll>x(n+1,0);  vector<ll>pre(n+1,0);
   for(i=0;i<n;i++){
      cin>>x[i];
   } map<ll,ll>vis;
   pre[0]=x[0];
   for(i=1;i<=n;i++) pre[i]=pre[i-1]+x[i];
   ll ans=0;
   ll reach=1; ll mxfreq=-1;
   for(i=0;i<=n;i++){
       if(!x[i]){
           (reach)? reach=0:ans+=mxfreq;   mxfreq=0;  vis.clear();
       }
       if(pre[i]==0 && reach){
           ans++;
       }
       vis[pre[i]]++; mxfreq=max(mxfreq,vis[pre[i]]);
   }
  // <=n >> to process last state
   cout<<ans<<endl;
