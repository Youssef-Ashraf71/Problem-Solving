...
{
    ll n,i=0,j=0,cnt=0;
    ll k ; cin>>n>>k; vector<ll>x(n); cin(x); ll sum=0;
    for(i=0;i<n;i++){
        sum+=x[i]/k; x[i]%=k;
    }
    multiset<ll>car(all(x));
    for(i=0;i<n;i++){
        ll cur = k-x[i];
        if(car.find(x[i])!=car.end()){
            car.erase(car.find(x[i]));
            auto it=car.lower_bound(cur);
            if(it!=car.end()){
                ll remov =(*it); car.erase(car.find(remov));  cnt++;
            }
        }
    }
    cout<<sum+cnt<<endl;
