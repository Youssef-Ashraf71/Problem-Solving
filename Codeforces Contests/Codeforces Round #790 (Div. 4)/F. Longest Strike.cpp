...
{
    ll n,i=0,j=0,cnt=0;
    ll k,l,r; cin>>n>>k; vector<ll>x(n); cin(x);
    map<ll,ll>vis; sort(all(x)); vector<ll>final;
    for(i=0;i<n;i++){
        vis[x[i]]++;
    }
    for(i=0;i<n;i++){
        if(vis[x[i]]>=k ) final.emplace_back(x[i]);
    }
    if(final.empty()){
        cout<<-1<<endl; return;
    } sort(all(final));
    ll  mx=0; ll tmp=l=r=final[0];
    for(i=1;i<final.size();i++){
        if(final[i]-final[i-1]<=1){
            ll length=final[i]-tmp;
            if(length>mx){
                l=tmp; r=final[i]; mx=length;
            }
        }
        else{
            tmp=final[i];
