...
{
    ll n,i,j,cnt=0;
    ll m; cin>>n; vector<ll>x(n); cin(x); cin>>m; vector<ll>y(m);cin(y);  sort(rall(x)) ,sort(rall(y));
    bool alice=0;
    for(i=0;i<min(x.size(),y.size());i++){
        if(x[i]<y[i]){ alice=0;  break;}
        if(x[i]>y[i]){ alice=1; break;}
        if(x[i]==y[i]){ alice=1; break;}
    }
    ...
    bool bob=0;
    for(i=0;i<min(x.size(),y.size());i++){
        if(x[i]<y[i]){ bob=1;  break;}
        if(x[i]>y[i]){ bob=0; break;}
        if(x[i]==y[i]){ bob=1; break;}
    }
