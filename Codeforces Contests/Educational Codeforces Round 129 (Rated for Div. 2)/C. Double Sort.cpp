...
{
    ll n,i,j,cnt=0;
    cin>>n; vector<ll>x(n),y(n); cin(x); cin(y);
    for(i=0;i<n;i++){
        if(x[i]!=y[i]) cnt++;
    }
    vector<ll>tmx(n),tmy(n); tmx=x; tmy=y; sort(all(tmx)); sort(all(tmy)); ll tmcnt=0;
    for(i=0;i<n;i++){
        if(tmx[i]!=tmy[i]) tmcnt++;
    }
    if(tmcnt!=cnt){
        cout<<-1<<endl; return;
    }
    ...
    for(i=0;i<n;i++){
        for(j=i+1;j<n;j++) {
            if (x[j] == tmx[i] && y[j] == tmy[i]) {
                swap(x[i], x[j]); swap(y[i], y[j]);
                ans.emplace_back(make_pair(i + 1, j + 1)); break;
            }
        }
    }
