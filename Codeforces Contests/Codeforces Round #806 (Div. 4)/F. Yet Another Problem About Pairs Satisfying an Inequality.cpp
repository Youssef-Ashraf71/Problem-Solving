...
void solve()
{
    ...
    for(i=1;i<=n;i++){
        if(x[i]>=i) continue;
        auto cur= lower_bound(all(idx),x[i]);
        ans+=cur-idx.begin();
        idx.push_back(i);
    }
    cout<<ans<<endl;
  ...
  // Hints
    /*
     * a[i]<i w a[j]<j
     * iti over el yenf3ao w 5od 3add el idxs
     *  geb kol el indices elly beyholdo
     *  shof lengtha kam w zawd 3al ans
     */
