...
void solve()
{
    ...
    vector<ll>ans(n); for(i=0;i<n;i++){ ans[i]= i+1; }   ll l=0,r=0;
    while (r<=n-1){
        while (x[r]==x[r+1]){
            if(r>n-2) break;  r++;
        }
        if(r==l){
            cout<<-1<<endl; return; // no dub values >> no answer exists
        }
        rotate(ans.begin()+l,ans.begin()+r,ans.begin()+r+1) ; // middle is the last as we are shifting to right
        l=r; l++; r++; // set l to the next segment
    }
    couth(ans); cout<<endl;
