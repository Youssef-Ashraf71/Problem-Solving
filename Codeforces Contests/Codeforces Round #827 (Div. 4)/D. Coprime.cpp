...
{
    ll n,i=0,j=0,cnt=0;
    cin>>n; vector<ll>x(n); cin(x); set<ll>unique; vector<ll>cur; ll ans(0);
    map<ll,ll>vis;
    for(i=0;i<n;i++){
        vis[x[i]]=i+1;
    }
    for(i=0;i<n;i++){
        if(unique.find(x[i])==unique.end()){
            unique.insert(x[i]); cur.push_back(x[i]);
        }
    }
    for(i=0;i<cur.size();i++){
        for(j=0;j<cur.size();j++){
            if(gcd(cur[i],cur[j])==1) ans=max(ans,vis[cur[i]]+vis[cur[j]]);
        }
    }
    (!ans) ? cout<<-1<<endl:cout<<ans<<endl;
}
int main()
{
