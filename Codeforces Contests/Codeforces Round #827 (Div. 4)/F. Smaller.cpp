...
void solve()
{
    ll n,i=0,j=0,cnt=0;
    cin>>n;
    string s,t; s=t="a"; ll len1,len2; len1=len2=1; bool swin,twin; swin=twin=0;
    for(i=0;i<n;i++){
        ll typ,k; cin>>typ>>k; string x; cin>>x;
        if(typ==1){
            for(j=0;j<x.size();j++){
                len1+=k;
                swin|=!(x[j]=='a');
        ...
        if(twin) { cout<<"YES"<<endl;  continue;  }
        if(swin) { cout<<"NO"<<endl; continue; ;}
        if(len1<len2) { cout<<"YES"<<endl; continue; ;}
        cout<<"NO"<<endl;
    }
}
int main()
