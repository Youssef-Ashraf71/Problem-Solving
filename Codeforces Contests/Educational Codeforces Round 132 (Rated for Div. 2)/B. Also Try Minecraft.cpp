...
{
    ll n,i=0,j=0,cnt=0;
    ll m ; cin>>n>>m; vector<ll>x(n); cin(x);
    vector<ll>dmgfr(n),dmgen(n); dmgfr[0]=0; dmgen[n-1]=0;
    for(i=1;i<n;i++){
        if(x[i]<x[i-1]) dmgfr[i]=x[i-1]-x[i];
        else dmgfr[i]=0;
    }
    for(i=n-2;i>=0;i--){
        if(x[i]<x[i+1]) dmgen[i]=x[i+1]-x[i];
        else dmgen[i]=0;
    }
    vector<ll>pre(n),suf(n);  pre[0]=dmgfr[0];  suf[0]=dmgen[0];
    for(i=1;i<n;i++){
        pre[i]=pre[i-1]+dmgfr[i];
    }
    for(i=n-2;i>=0;i--){
        suf[i]=suf[i+1]+dmgen[i];
    }
  while (m--){
      ll l,r; cin>>l>>r; l--;r--;
      if(l<r){
        if(l==0){
            cout<<pre[r]-pre[0]<<endl;
        }
        else{
