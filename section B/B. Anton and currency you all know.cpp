...
void solve()
{
    ...
    for(i=0;i<s.size();i++){
        if((s[i]-'0')%2==0)  { ok=1 ; break;  }
    }
    if(!ok) { cout<<-1<<endl; return; }
    vector<string>x; string mx; mx="";
    for( i=0;i<s.size()-1;i++){
        if((s[i]-'0')%2) continue;
        if(s[i]>s[s.size()-1]) continue;
        string tmp=s; swap(tmp[i],tmp[s.size()-1]);
        mx=max(mx,tmp); cout<<mx<<endl; return;
    }
    for(i=s.size()-2;i>=0;i--){
       if((s[i]-'0')%2==0){
           swap(s[i],s[s.size()-1]); cout<<s<<endl; return;
       }
    }
