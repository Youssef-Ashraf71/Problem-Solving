...
{
    ll n,i,j,cnt=0;
    ll x,y,a,b; cin>>n>>x>>y>>a>>b; ll dia=n*2;
    if(x==a && y==b){ cout<<0<<endl; return; }
    double  dist=sqrt(llabs(x-a)*llabs(x-a)+llabs(y-b)*llabs(y-b));
    ll ans= dist/dia; if( ans<((double )dist/(double )dia) )  ans++;
    cout<<ans<<endl;
